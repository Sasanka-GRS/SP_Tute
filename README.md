...
10. Lines 152-192 are for the case when the size of the file is greater than the maxSize (=10KB) where the file is read, reversed and written using a sliding window of lenght = maxSize. The logic works as follows:
  ...
  - 1 specific case to deal with includes the case when the number of characters in the file to be read from is not a multiple of the maxSize. In this case, there would be some r (= size%maxSize) charcters left over which can be read separately character by character and written.
